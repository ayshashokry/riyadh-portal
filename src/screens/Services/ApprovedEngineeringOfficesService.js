...
export default class ApprovedEngineeringOfficesService extends Component {
  constructor(props) {
    super(props);
    this.state = {
      formValues: {
  ...
  handleChangePage = (page) => {
    ...
    if (
      ...
      axios
        .get(
          window.ApiUrl +
            `/EngineeringCompany/GetAllEngCompApproved?page=${
              page - 1
            }&pageSize=30`
        )
        .then((res) => {
  ...
  async componentDidMount() {
    ...
    await new Promise((resolve, reject) => {
      axios
        .get(
          window.ApiUrl +
            `/EngineeringCompany/GetAllEngCompApproved?pageSize=30`
        )
        .then((res) => {
          resolve(res.data);
          if (res.data) {
      ...
      <div className="goalsPage approvedServie">
        ...
        <div className="goalsHeader">
          <div className=" pt-5 pb-3 px-5 mr-lg-5">
            ...
            <span className="span2">.  المكاتب الهندسية المعتمدة</span>
            <h1> المكـاتب الهندسيـة المعتمـدة</h1>
          </div>
        </div>
                          ...
                          >
                            <thead>
                              <tr>
                                ...
                                <th className="thDep">
                                  الإدارة المقدم عليها لطلب الاعتماد
                                </th>
                                <th>الفئة </th>
                              ...
                              {this.state.tableResult !== undefined &&
                                ...
                                this.state.tableResult.map((res, index) => (
                                  ...
                                  >
                                    ...
                                    <td>{res.email}</td> <td>{res.name}</td>
                                  </tr>
                                ))}
                            </tbody>
                          </Table>
